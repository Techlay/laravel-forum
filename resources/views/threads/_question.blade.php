...
<modal name="update-thread" width="60%" height="auto" v-cloak>
    <div class="p-6 py-8">
        <div class="mb-6 -mx-4">
            <div class="px-4 mb-6">
            ...
            <p class="text-xs text-grey-darker mb-4">
                ...
                <span v-if="! editing">
                    <span v-if="(authorize('isAdmin') || authorize('owns', thread))">
                        <a href="#" class="text-blue link pl-2 ml-2 border-l" @click.prevent="editing = true">
                            Edit
                        </a>

                        <span v-if="authorize('isAdmin')">
                            <a href="#" class="link pl-2 ml-2 border-l"
                               :class="locked ? 'font-bold' : ''"
                               @click.prevent="toggleLock"
                               v-text="locked ? 'Unlock' : 'Lock'"></a>
                            <a href="#" class="link pl-2 ml-2 border-l"
                               :class="pinned ? 'font-bold' : ''"
                               @click.prevent="togglePin"
                               v-text="pinned ? 'Unpin' : 'Pin'"></a>
                        </span>
                    </span>
                    <subscribe-button
